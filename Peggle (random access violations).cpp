...
#define MOVER_VERTICAL 1
#define MOVER_HORIZONTAL 2
#define MOVER_VERTICALHORIZONTAL 3
#define MOVER_INFINITY 4
...
void peggle_activate_ability(const v8::FunctionCallbackInfo<v8::Value>& info) {
    ...
    if (info[0]->IsNumber()) {
        ...
        unknown1 = IntegerFI(info[0].As<Object>()->Get(isolate->GetCurrentContext(), LITERAL("location")).ToLocalChecked());
    }
    if (info[1]->IsNumber()) {
        unknown2 = IntegerFI(info[1]);
    }
    else if (info[1]->IsObject()) {
        unknown2 = IntegerFI(info[1].As<Object>()->Get(isolate->GetCurrentContext(), LITERAL("location")).ToLocalChecked());
    }
...
namespace Templates {
    ...
    void Init(v8::Isolate* isolate) {
        ...
        FloatProperty(_Ball, "multiball", 0xE8); //idk what it does exactly
        ...
        //hmm it could also be the animation frame but the peg sparkle effects have a different amount of frames so wouldn't that mean the ones with more frames would be faster? (im not gonna lie it looks like this could be true, the blue pegs have the least frames and they seem to play the fastest too)
        //it actually might be my earlier guesses might've been wrong!
        //Property(_PegInfo, "lengthOfTimeThisPegWasLastHit", 0x20); //for some reason there's 2 (oh wait) (ah something special about this one is that it doesn't go above 100 because im assuming something happens when you do (the pegs prolly disappear))
        Property(_PegInfo, "animationPercent", 0x20);
        ...
        //Property(_Hole, "time", 0x4C); //also not sure
        //
        //_Mover = GenericPtrObject(isolate, nullptr);
        //Property(_Mover, "type", 0x10); //confirmed
        //Property(_Mover, "idk", 0x14);
        //Property(_Mover, "somethingtime", 0x18);
        //Property(_Mover, "important", 0x1C);
        //FloatProperty(_Mover, "offset", 0x20); //confirmed
...
//#error LogicMgr will be invalid if we choose a new level, i'd have to re call init peggle globals
    ...
    ConstProperty(global, MOVER_VERTICAL);
    ConstProperty(global, MOVER_HORIZONTAL);
    ConstProperty(global, MOVER_VERTICALHORIZONTAL);
    ConstProperty(global, MOVER_INFINITY);
    ...
    //    ULONG_PTR esi = (**(ULONG_PTR**)(base + 0x194)); // + 0x190;
